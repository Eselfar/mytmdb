 ...
 */
class InMemoryByPageKeyRepository(private val tmDbService: TMDbService,
                                  private val networkExecutor: Executor) : ResultsRepository {
    @MainThread
    override fun nowPlayingResults(pageSize: Int): Listing<Result> {
        val sourceFactory = ResultDataSourceFactory(tmDbService, networkExecutor)
